...
public class Calculator {
     ...
     */
    static int executeInstructions(List<Pair<Operation, Integer>> instructions){
        if(instructions.isEmpty()) throw new IllegalStateException("List witch instructions cannot be empty");
        if(instructions.get(0).getLeft() != Operation.apply) throw new IllegalStateException("First instruction must be apply");

        int result = instructions.get(0).getRight();

        for (Pair<Operation, Integer> instruction : instructions) {
            switch (instruction.getLeft()){
                case add: result += instruction.getRight(); break;
                case multiply: result *= instruction.getRight(); break;
                case subtract: result -= instruction.getRight(); break;
                case divide: result /= instruction.getRight(); break;
            }
        }
     ...
     */
     static List<Pair<Operation, Integer>> getInstructionsFromFile(File calculationsFile){
